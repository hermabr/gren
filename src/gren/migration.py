...
MigrationValue: TypeAlias = (
    Primitive | Gren | tuple["MigrationValue", ...] | dict[str, "MigrationValue"]
)
...
def find_migration_candidates_initialized_target(
    *,
    ...
    target_config = GrenSerializer.to_dict(to_obj)
...
def _find_dependents(candidate: MigrationCandidate) -> list[MigrationCandidate]:
    ...
    for ref, config in _iter_all_configs():
        ...
        if (
            dependent_candidate.to_ref.gren_hash
            == dependent_candidate.from_ref.gren_hash
        ):
            continue
...
def _replace_dependency(
    ...
    if isinstance(value, dict):
        if "__class__" in value:
            if GrenSerializer.compute_hash(value) == from_hash:
...
def _apply_single_migration(
    ...
    *,
    ...
    to_obj = GrenSerializer.from_dict(candidate.to_config)
    ...
    record = MigrationRecord(
        ...
        to_hash=candidate.to_ref.gren_hash,
    ...
    event: dict[str, str | int] = {
        "type": "migrated",
        "policy": policy,
        "from": f"{candidate.from_ref.namespace}:{candidate.from_ref.gren_hash}",
        "to": f"{candidate.to_ref.namespace}:{candidate.to_ref.gren_hash}",
    }
...
def _iter_source_configs(
    namespace: str,
) -> Iterable[tuple[GrenRef, dict[str, JsonValue]]]:
    namespace_path = Path(*namespace.split("."))
    for version_controlled in (False, True):
        root = GREN_CONFIG.get_root(version_controlled=version_controlled)
        ...
        for entry in class_dir.iterdir():
            ...
            ref = GrenRef(
                namespace=namespace,
                gren_hash=entry.name,
                root=root_kind,
                directory=entry,
            )
            yield ref, _typed_config(gren_obj)


def _iter_all_configs() -> Iterable[tuple[GrenRef, dict[str, JsonValue]]]:
    for version_controlled in (False, True):
        root = GREN_CONFIG.get_root(version_controlled=version_controlled)
        if not root.exists():
            continue
...
def _build_candidate(
    ...
    *,
    ...
    to_hash = GrenSerializer.compute_hash(to_config)
...
def _target_field_names(target_class: type[Gren]) -> list[str]:
...
def _serialize_value(value: MigrationValue) -> JsonValue:
    result = GrenSerializer.to_dict(value)
    if result is None:
        return result
...
def _typecheck_config(config: dict[str, JsonValue]) -> None:
    obj = GrenSerializer.from_dict(config)
    obj = _normalize_tuple_fields(obj)
    for_all_fields(typecheck)(obj)


def _normalize_tuple_fields(obj: Gren) -> Gren:
...
def _build_target_ref(
    ...
    root = GREN_CONFIG.get_root(version_controlled=target_class.version_controlled)
    directory = root / Path(*namespace.split(".")) / gren_hash
    return GrenRef(
        namespace=namespace,
        gren_hash=gren_hash,
        root=root_kind,
...
def _resolve_target_class(namespace: str) -> type[Gren]:
    ...
    if obj is None:
        raise ValueError(f"migration: unable to resolve target class: {namespace}")
    if not _is_gren_class(obj):
        raise ValueError(f"migration: unable to resolve target class: {namespace}")
    return cast(type[Gren], obj)


def _is_gren_class(value: object) -> bool:
    return isinstance(value, type) and issubclass(value, Gren)


def _namespace_str(target_class: type[Gren]) -> str:
